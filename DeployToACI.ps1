...
az container create --resource-group $resourceGroup --name $containerName --image $containerImage --os-type $osType --cpu $cpus --memory $memory --registry-login-server $acrloginserver --registry-username $acrUsername --registry-password $acrPassword --dns-name-label $appDns --ports $appPort
